...
class ScrappingRecetasFacilController extends BaseController {
...
//	canalcocina.es
	...
	public function getRecipes2()
	{
		$ingredientsDB = DB::table('ingredients')
			...
			->get();
		for($n = 1; $n<=5; $n++){
			$url_busqueda = "http://www.recetasfacil.com/recipes.php?id=20&p=".$n."&orderbypost_date&sortbydesc&recipe_category=Cerdo";
...
//				}
	...
	public function getRecipes()
	{
		...
		for($n =1; $n<=5; $n++){
			$url_busqueda = "http://www.recetasfacil.com/recipes.php?id=17&p=".$n."&orderbypost_date&sortbydesc&recipe_category=Pasta,_Canelones,_Lasa%F1a";
			$new_fichero = file_get_contents($url_busqueda);
			$more_recipes = true;
			$urls = array();
			$i = 0;
			while($more_recipes and $i<10){
				$pos = strpos($new_fichero, "rctcntbx");
				$new_fichero = substr($new_fichero, $pos, strlen($new_fichero));

				$pos_href = strpos($new_fichero, "href");
				$new_fichero = substr($new_fichero, $pos_href+6, strlen($new_fichero));
				$pos_end_href = strpos($new_fichero, '"');
				$url = "http://www.recetasfacil.com".substr($new_fichero,0,$pos_end_href);
				$new_fichero = substr($new_fichero, $pos_end_href, strlen($new_fichero));

				$pos_title = strpos($new_fichero, "title");
				$new_fichero = substr($new_fichero, $pos_title+17, strlen($new_fichero));
				$pos_end_title = strpos($new_fichero, '"');
				$title = substr($new_fichero,0,$pos_end_title);
				$new_fichero = substr($new_fichero, $pos_end_title, strlen($new_fichero));
				...
				$new_fichero = substr($new_fichero, $pos_img+5, strlen($new_fichero));
				$pos_end_img = strpos($new_fichero, '"');
				$foto = "http://www.recetasfacil.com/".substr($new_fichero,0,$pos_end_img);
				$new_fichero = substr($new_fichero, $pos_end_img, strlen($new_fichero));
			...
			foreach($urls as $url){
				...
				$more_ingredients = true;
				$ingredients = array();
				$pos_ingredients_list = strpos($new_fichero, "<h2>Ingredientes</h2>");
				$new_fichero = substr($new_fichero, $pos_ingredients_list, strlen($new_fichero));
				$pos_ingredient = strpos($new_fichero, 'ingredients">');
				$new_fichero = substr($new_fichero, $pos_ingredient+11, strlen($new_fichero));
				while($more_ingredients){
					$pos_end_ingredient = strpos($new_fichero, '<br />');
					$ingredient = substr($new_fichero,0,$pos_end_ingredient);
					$new_fichero = substr($new_fichero, $pos_end_ingredient+6, strlen($new_fichero));
					$pos_next = strpos($new_fichero, '<br />');
					$pos_next2 = strpos($new_fichero, '</td>');
					if($ingredient != "\r\n"){
						array_push($ingredients, $ingredient);
					}
					if(!$pos_next || $pos_next>$pos_next2){
						$more_ingredients = false;
					}
				}
				$pos_img = strpos($new_fichero, '<span itemprop="image"');
				$new_fichero = substr($new_fichero, $pos_img, strlen($new_fichero));
				$pos_img = strpos($new_fichero, '<img src="');
				$new_fichero = substr($new_fichero, $pos_img+10, strlen($new_fichero));
				$pos_end_img = strpos($new_fichero, '"');
				$foto = substr($new_fichero,0,$pos_end_img);
				if($foto != "td width="){
					$url['image_url'] = $foto;
				...
				$new_fichero = substr($new_fichero, $pos_time, strlen($new_fichero));
				$pos_time = strpos($new_fichero, '</h2>');
				$new_fichero = substr($new_fichero, $pos_time+9, strlen($new_fichero));
				$pos_end_time = strpos($new_fichero, ' minutos');
				$time = substr($new_fichero,0,$pos_end_time);
				if($time == ""){
					$time = 0;
				}
